...
def influential(file,start_frame,num_frames,fish_pair):
    data_file = file
    ...
    w = 4  #window size, how many frames will be averaged to check for correlation
    ...
    if fish_pair == "1to2":
        ...
        for t_k in range(len(fish2_velocities) - w):  
            
            correlation = 0.0 #just initalises the correlation, can be ignored
            for tau_r in range(min(t_k,max_time_delay)):
                
                # Calculate C_ij(t_k, tau_r, w) 
                start_index = t_k 
                end_index = t_k + w
                current_velocities = fish2_velocities[start_index:end_index]
                shifted_velocities = fish1_velocities[start_index - tau_r:end_index - tau_r]
                current_velocities = current_velocities.astype(float)  
                shifted_velocities = shifted_velocities.astype(float)
