...
xTest = xTestLoad[4300:4600]
yTest = yTestLoad[4300:4600]
...
model = load_model("5FishAllModelVel4")
...
for index in range(len(x1Frames)):
  ...
  if index < 1:
    ...
    predCache = np.squeeze(model.predict(cache.reshape((1,20))))
...
def plot1D(k):
  ...
  if k < 30:
    ...
    ax1.scatter(x2Frames[:k], y2Frames[:k], color = "blue", s = 1, label = "Real Fish")
    ...
    ax1.scatter(predictedX[:k], predictedY[:k], color = "red", s = 1, label = "Fake Fish")

    plt.legend()
    return plt

  else:
    ax1.scatter(x1Frames[k-30:k], y1Frames[k-30:k], color = "green", s = 1, label = "Real Fish")
    ax1.scatter(x2Frames[k-30:k], y2Frames[k-30:k], color = "blue", s = 1, label = "Real Fish")
    ...
    ax1.scatter(predictedX[k-30:k], predictedY[k-30:k], color = "red", s = 1, label = "Fake Fish")
